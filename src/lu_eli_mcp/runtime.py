...
Pulls a tiny static ``lu-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/lu-runtime.json
...
Opt-out: set ``LU_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "LU_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/lu-eli-mcp/releases/latest/download/lu-runtime.json.gz",
)
_CACHE_NAME = "lu-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "lu-eli-mcp-runtime (+https://github.com/matematicsolutions/lu-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("LU_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "lu-eli"
