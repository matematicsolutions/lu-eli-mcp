...
SOURCE = 'data.legilux.public.lu (jolux RDF, ELI-native)'
...
_FAMILIES: list[dict] = [{'name': 'Legislation', 'tool': 'lu_get_act / lu_get_text'}]

_GAPS: list[dict] = [{'id': 'LU-001', 'family': 'Legislation', 'missing': 'There is no HTTP search endpoint, so discovery is by ELI coordinate only - no free-text search.', 'fallback': 'Obtain an ELI from the Legilux website, then query here.'}, {'id': 'LU-002', 'family': 'Legislation', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
