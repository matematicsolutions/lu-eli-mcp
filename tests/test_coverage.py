...
from lu_eli_mcp.coverage import build_coverage
from lu_eli_mcp.server import lu_coverage


def _coverage():
    fn = getattr(lu_coverage, "fn", lu_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LU_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("lu_coverage" in line for line in lines), (
        "an audit entry was written but it does not name lu_coverage"
    )
